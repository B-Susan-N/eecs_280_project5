...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static void destroy_nodes_impl(Node *node) {
    ...
    if (!node){
      ...
      delete temp;
      destroy_nodes_impl(temp_right);
      destroy_nodes_impl(temp_left);
     }
}
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
    ...
    if (!node) {
      ...
      Node *left_inserting = node->left;
      node->left = insert_impl(left_inserting, item, less); 
    }
    else {
      Node *right_inserting = node->right;
      node->right = insert_impl(right_inserting, item, less);
    }
    return node;
  }
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    ...
    if(node->left){
      if(less(node->datum, node->left->datum) && (less(node->datum, max_element_impl(node->left)->datum))){
        return false;
      }
    }
    if(node->right){
      if(less(node->right->datum, node->datum) && (less(min_element_impl(node->right)->datum, node->datum))){
        return false;
      }
    } 
